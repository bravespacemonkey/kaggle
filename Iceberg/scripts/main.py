...
callbacks = [
    ...
    ModelCheckpoint(os.path.join(base_path, "best_val_weights"), monitor='val_loss', save_best_only=True, verbose=1)
]

history = simple_cnn.fit(X_train, y_train, validation_data=(X_test, y_test), epochs=100, shuffle=True, callbacks=callbacks)

simple_cnn.load_weights(os.path.join(base_path, "best_val_weights"))

test_predictions = simple_cnn.predict(test_images)

pred_df = test_df[['id']].copy()
pred_df['is_iceberg'] = test_predictions[:, 1]
pred_df.to_csv(os.path.join(base_path, 'predictions.csv'), index=False)
